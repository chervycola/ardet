...
> **Re-spec 2026-05/07 (Decision 12).** Vactrol crossfader (старая архитектура) снят с этого слота. Полная канoническая техспека модуля живёт на ветке `origin/claude/jolly-gates-KxAP2` в директории `is_my/` — см. также локальную копию в `03_is_my/` этой ветки. Ниже — концептуальный обзор.
...
> Exciter — **универсальный DAEX32Q-4 в модуле** (Decision 11), картридж пассивный. Размеры refined (acoustic-optimized, см. `../01_last_night/acoustic_modeling.md §10`).
