/* SYSTEM SUICIDE — i18n.
   Tiny in-house translation layer: dictionary + React context + hook +
   language-toggle component. Persists choice in localStorage. Wraps the
   whole app via <LangProvider>. */
...
const TRANSLATIONS = {
  ...
  en: {
    ...
    ln_desc:           "An analogue reverb with no DSP. A surface exciter drives a thin plate held in a cartridge — wood, stone, metal, glass, bone, jade. Two piezo pickups read the resonance from the other face. A solenoid damps. A feedback loop freezes. Each cartridge is a different room.",
    ...
    man_p1:            "We make instruments. Each one has a body — a thing that resonates, vibrates, heats, decays. The circuit board exists to feed the body and listen to its answer. When the body sings, the circuit gets out of the way.",
    man_p2_part1:      "No DSP. No screens. No presets.",
    man_p2_part2:      " The voice of the instrument is the voice of its material — wood, brass, glass, oil, light, skin. Each cartridge is a different room. Each pendulum keeps its own time. Each plate ages and the sound ages with it.",
    man_p3:            "We ship slowly. We replace what breaks. We refuse to certify anything as silent.",
    ...
    pp_embargo:        "This module is under embargo. Details — physical core, signal chain, controls and exchange components — are withheld until ship.\nFor pre-order interest, write to {email}.",
    pp_h_specs:        "Specs",
  ...
  ru: {
    ...
    ln_desc:           "Аналоговый реверб без DSP. Surface-эксайтер раскачивает тонкую пластину в картридже — дерево, камень, металл, стекло, кость, нефрит. Два пьезо-датчика снимают резонанс с обратной стороны. Соленоид демпфирует. Feedback-петля замораживает. Каждый картридж — другая комната.",
    ...
    man_p1:            "Мы делаем инструменты. У каждого есть тело — то, что резонирует, вибрирует, нагревается, распадается. Плата существует, чтобы кормить тело и слушать его ответ. Когда тело поёт, схема уходит с дороги.",
    man_p2_part1:      "Никаких DSP. Никаких экранов. Никаких пресетов.",
    man_p2_part2:      " Голос инструмента — это голос материала: дерево, латунь, стекло, масло, свет, кожа. Каждый картридж — другая комната. Каждый маятник держит своё время. Каждая пластина стареет, и звук стареет вместе с ней.",
    man_p3:            "Мы отгружаем медленно. Заменяем то, что ломается. Отказываемся сертифицировать что-либо как «бесшумное».",
    ...
    pp_embargo:        "Модуль под эмбарго. Детали — физическое ядро, сигнальная цепь, контролы и сменные компоненты — закрыты до отгрузки.\nДля предзаказа пишите на {email}.",
...
function LangProvider({ children }) {
  const [lang, setLang] = useState(() => {
    try { return localStorage.getItem("ss_lang") || "en"; } catch { return "en"; }
  });
  useEffect(() => {
...
Object.assign(window, {
  LangContext, LangProvider, TRANSLATIONS,
  useT, useLang, LanguageToggle,
});
