/* SVG motif primitives for SYSTEM SUICIDE.
   Each is a vertical rail or strip — used as page rails, dividers, counters.
   Pure CSS color: var(--accent) drives the red. */

const { useMemo } = React;

// ============================================================
// VERTICAL RAILS — fixed elements on left/right page edges
// ============================================================

function RailRazor({ side }) {
  // a single hairline scratch with periodic micro-nicks
  return (
    <svg width="80" height="100%" viewBox="0 0 80 1000" preserveAspectRatio="none" aria-hidden="true">
      <line x1="40" y1="0" x2="40" y2="1000"
        stroke="var(--line)" strokeWidth="1" />
      {Array.from({length: 22}).map((_, i) => {
        const y = i * 48 + 20;
        const len = (i % 3 === 0) ? 14 : 6;
        return (
          <line key={i} x1={40 - len/2} y1={y} x2={40 + len/2} y2={y}
            stroke={i % 5 === 0 ? "var(--accent)" : "var(--fg-3)"} strokeWidth="1" />
        );
      })}
    </svg>
  );
...
function RailRope({ side }) {
  // wiggly red strand resembling a thin frayed rope
  const path = useMemo(() => {
    const pts = [];
    for (let y = 0; y < 1000; y += 16) {
      const x = 40 + Math.sin(y / 28 + (side === "right" ? 2 : 0)) * 3.5
                    + Math.sin(y / 7) * 1.2;
    ...
    <svg width="80" height="100%" viewBox="0 0 80 1000" preserveAspectRatio="none" aria-hidden="true">
      ...
      {Array.from({length: 38}).map((_, i) => {
        ...
        return (
          <line key={i}
            x1={x} y1={y}
            x2={x + dx} y2={y + (i % 3) * 1.5 - 1}
            stroke="var(--accent-dim)" strokeWidth="0.6" opacity="0.7" />
        );
      })}
    </svg>
  );
...
function RailX({ side }) {
  // a vertical line of small red X marks
  return (
    <svg width="80" height="100%" viewBox="0 0 80 1000" preserveAspectRatio="none" aria-hidden="true">
...
function RailBarbed({ side }) {
  return (
    <svg width="80" height="100%" viewBox="0 0 80 1000" preserveAspectRatio="none" aria-hidden="true">
      {/* twisted wire = two phase-shifted lines */}
      {[0, 1].map(k => {
        const path = [];
        ...
        return <path key={k} d={path.join(" ")} stroke="var(--accent)" strokeWidth="1.2" fill="none" opacity="0.85" />;
      })}
      {/* barbs */}
  ...
  return (
    <svg width="80" height="100%" viewBox="0 0 80 1000" preserveAspectRatio="none" aria-hidden="true">
      ...
      {Array.from({length: 28}).map((_, i) => {
        ...
        const inGroup = i % 5;
        if (inGroup < 4) {
          return <line key={i} x1={40 - 9} y1={y} x2={40 - 9 + 6} y2={y - 22}
            stroke="var(--fg-2)" strokeWidth="1.2" />;
        }
        return <line key={i} x1={40 - 14} y1={y - 18} x2={40 + 8} y2={y - 4}
...
// Vertical SYSTEM word repeated down the rail.
...
// Animated rope dangling from the top with a hangman's knot.
// The loop hangs far below the visible viewport — clipped so it never reads
// as a tidy oval; the rope appears to disappear into the page below.
    ...
    <div className="rail-rope-anim" aria-hidden="true">
      <div className="rope-swing">
        <svg viewBox="0 0 80 2200" preserveAspectRatio="xMidYMin meet">
          ...
          {/* hangman's knot — coil bundle, set lower for realism */}
          <g transform="translate(40, 720)">
            <path d="M -11 0 Q -14 18 -11 36 L 11 36 Q 14 18 11 0 Z"
                  fill="oklch(0.16 0.04 25)" stroke="var(--accent)" strokeWidth="1.6" />
            <line x1="-2" y1="0" x2="-2" y2="36" stroke="oklch(0.30 0.10 25)" strokeWidth="1" />
            {Array.from({length: 8}).map((_, i) => {
              const y = 3 + i * 4;
              return <line key={i} x1="-11" y1={y} x2="11" y2={y + 3.2}
                stroke="var(--accent)" strokeWidth="1.4" />;
            })}
          ...
          {/* working end — continues down past the visible bottom; clipped */}
          {(() => {
            const pts = [];
            for (let y = 756; y <= 2200; y += 12) {
              const x = 40 + Math.sin(y / 48) * 1.4;
              pts.push(`${y === 616 ? "M" : "L"}${x.toFixed(2)},${y}`);
            }
            return <path d={pts.join(" ")} stroke="var(--accent)" strokeWidth="1.8" fill="none" opacity="0.85" />;
          })()}

          {/* loop sits far below the viewport, never shown */}
...
const RAIL_MAP = {
  razor: RailRazor, rope: RailRope, x: RailX, barbed: RailBarbed, tally: RailTally,
  system: RailSystemVert, "rope-anim": RailRopeAnim,
};

function Rails({ left = "razor", right = "razor", enabled = true }) {
  if (!enabled) return null;
  const L = RAIL_MAP[left] || RailRazor;
  const R = RAIL_MAP[right] || RailRazor;
  return (
    <div className="rails" aria-hidden="true">
      <div className="rail left"><L side="left" /></div>
      <div className="rail right"><R side="right" /></div>
    </div>
  );
}

// ============================================================
// HORIZONTAL STRIPS — counter row, footer divider
// ============================================================

function TallyCounter({ count = 47 }) {
  // 47 marks = 9 full groups + 2 leftovers
  const groups = Math.floor(count / 5);
  const rest = count % 5;
  return (
    <svg viewBox="0 0 480 36" preserveAspectRatio="xMinYMid meet" aria-hidden="true" style={{width:"100%",height:36}}>
      {Array.from({length: groups}).map((_, g) => {
        const x0 = g * 40 + 4;
        return (
          <g key={g}>
            {[0,1,2,3].map(k => (
              <line key={k} x1={x0 + k*6} y1={4} x2={x0 + k*6 + 3} y2={32}
                stroke="var(--fg)" strokeWidth="1.4" />
            ))}
            <line x1={x0 - 2} y1={28} x2={x0 + 26} y2={8}
              stroke="var(--accent)" strokeWidth="1.6" />
          </g>
        );
      })}
      {Array.from({length: rest}).map((_, k) => {
        const x0 = groups * 40 + 4 + k * 6;
        return <line key={"r"+k} x1={x0} y1={4} x2={x0 + 3} y2={32}
          stroke="var(--accent)" strokeWidth="1.4" />;
      })}
    </svg>
  );
...
function BarbedDivider() {
  return (
    <svg viewBox="0 0 1200 36" preserveAspectRatio="none" aria-hidden="true" style={{width:"100%",height:36}}>
      {[0, 1].map(k => {
        const path = [];
...
function RazorMark({ size = 18 }) {
  // small inline mark — vertical line with one nick (used as bullet)
  return (
...
// ============================================================
// RAZOR BLADE — visual stand-in for the word "suicide"
// ============================================================
let _bladeIdCounter = 0;
const nextBladeId = () => `blade-${++_bladeIdCounter}`;

// Full razor blade silhouette with the classic twin dumbbell slots.
// Used at hero scale.
function RazorBlade({ color = "var(--accent)", className, style }) {
  ...
  return (
    ...
    >
      ...
      {/* light bevel highlight along top edge */}
      <line x1="80" y1="34" x2="1120" y2="34" stroke="rgba(255,255,255,0.18)" strokeWidth="1.5" />
      {/* shadow line along bottom */}
      <line x1="80" y1="186" x2="1120" y2="186" stroke="rgba(0,0,0,0.35)" strokeWidth="1.5" />
    </svg>
  );
}

// Mid scale — a horizontal slash with a thin razor body + one dumbbell slot.
// Used in footer brand and other contexts roughly the size of a word.
function RazorSlash({ color = "var(--accent)", className, style }) {
  const id = useMemo(() => nextBladeId(), []);
  return (
    <svg
      viewBox="0 0 800 120"
...
// Tiny inline scratch — a quick horizontal cut with one nick.
// Used in topbar after the word SYSTEM.
function RazorScratch({ width = 80, color = "var(--accent)", style }) {
  return (
    <svg
      viewBox="0 0 80 14"
      width={width}
...
// ============================================================
// AMEX-STYLE RAZOR BLADE — hero centerpiece
// ============================================================

function AmexRazorBlade({ className, style }) {
  const id = useMemo(() => nextBladeId(), []);
  ...
  return (
    ...
    >
      <defs>
        {/* main body gradient — brushed steel with cool blue-grey tint */}
        ...
        <mask id={cutMaskId} maskUnits="userSpaceOnUse" x="0" y="0" width="1400" height="500">
          ...
          {/* left dumbbell slot — proper razor-blade proportions:
              circles bigger than the bar between them */}
          <g fill="black">
      ...
      {/* OUTER STICKER OUTLINE — light steel halo around the body */}
      <path
        d="
          M 100 70
      ...
      {/* OUTER BODY — rounded rectangle with four small corner tabs
         Path traverses clockwise starting at the top-left rounded corner. */}
      <path
        d="
          M 110 80
      ...
      {/* polished bevel highlights along the long edges */}
      <line x1="130" y1="92" x2="1270" y2="92"
        stroke="rgba(220,232,240,0.50)" strokeWidth="1.6" />
      <line x1="130" y1="408" x2="1270" y2="408"
        stroke="rgba(0,0,0,0.62)" strokeWidth="1.6" />
      {/* inner frame line — card-style double border, more visible */}
      <rect x="82" y="100" width="1236" height="300"
        fill="none" stroke="oklch(0.60 0.012 230)" strokeWidth="1"
        rx="6" />
      <rect x="88" y="106" width="1224" height="288"
        fill="none" stroke="rgba(0,0,0,0.55)" strokeWidth="0.6"
        rx="4" />
      {/* subtle thinner reflections */}
      <line x1="150" y1="100" x2="1250" y2="100"
        stroke="rgba(220,232,240,0.22)" strokeWidth="0.6" />

      {/* SLOT OUTLINES — polished steel edges */}
      ...
      {/* TOP WORDMARK — embossed steel */}
      <g className="amex-word">
        <text
          x="700" y="171"
          textAnchor="middle"
          fontFamily="Archivo Narrow, Archivo Black, sans-serif"
          fontWeight="700"
          fontSize="56"
          letterSpacing="28"
          fill="rgba(220,232,240,0.40)"
        >
          YOUR CARD
        </text>
        <text
        ...
        <text
          ...
          fontSize="56"
          letterSpacing="28"
          fill="oklch(0.90 0.012 230)"
        >
          YOUR CARD
        </text>
      </g>

      {/* CENTREPIECE MEDALLION — coin-like detail */}
      <g transform="translate(700, 250)">
        {/* outer ring with brighter rim */}
        <ellipse cx="0" cy="0" rx="66" ry="86"
          fill={`url(#${portraitGradId})`}
          stroke="oklch(0.62 0.005 60)" strokeWidth="2" />
        {/* inner highlight ring */}
        <ellipse cx="0" cy="0" rx="62" ry="82"
          fill="none"
          stroke="rgba(255,255,255,0.20)" strokeWidth="0.6" />
        {/* inner thin ring */}
        <ellipse cx="0" cy="0" rx="56" ry="76"
          fill="none"
          stroke="oklch(0.52 0.005 60)" strokeWidth="0.8" />
        {/* concentric decorative arc — coin guilloché */}
        <ellipse cx="0" cy="0" rx="50" ry="70"
          fill="none"
          stroke="oklch(0.46 0.005 60)" strokeWidth="0.5"
          strokeDasharray="2 2.5" />
        {/* perimeter micro-text */}
        <defs>
        ...
        <text
          ...
          fontSize="8"
          letterSpacing="3"
          fill="oklch(0.62 0.005 60)"
        >
          <textPath href={`#${id}-arc`} startOffset="50%" textAnchor="middle">
            · PHYSICAL · SYNTHESIS ·
          </textPath>
        </text>
        {/* faint Roman-helmet silhouette behind the IX (just a crest + dome hint) */}
        <g opacity="0.16">
          {/* dome */}
          <path d="M -34 -10 Q -34 -40 0 -40 Q 34 -40 34 -10 L 34 6 L -34 6 Z"
            fill="oklch(0.62 0.005 60)" />
          {/* crest line on top */}
          <path d="M -10 -42 Q 0 -52 10 -42 Q 0 -36 -10 -42 Z"
            fill="oklch(0.62 0.005 60)" />
          {/* cheek lines */}
          <line x1="-30" y1="0" x2="-28" y2="22" stroke="oklch(0.62 0.005 60)" strokeWidth="2" />
          <line x1="30" y1="0" x2="28" y2="22" stroke="oklch(0.62 0.005 60)" strokeWidth="2" />
        </g>
        {/* roman numeral IX — embossed serif */}
        <text x="0" y="22" textAnchor="middle"
          ...
          fontSize="64"
          letterSpacing="4"
          fill="rgba(220,232,240,0.40)">AX</text>
        <text x="0" y="25" textAnchor="middle"
          fontFamily="Georgia, Cormorant Garamond, Times New Roman, serif"
          fontWeight="700"
          fontSize="64"
          letterSpacing="4"
          fill="rgba(0,0,0,0.72)">AX</text>
        <text x="0" y="23" textAnchor="middle"
          fontFamily="Georgia, Cormorant Garamond, Times New Roman, serif"
          fontWeight="700"
          fontSize="64"
          letterSpacing="4"
          fill="oklch(0.88 0.012 230)">AX</text>
        {/* tiny red razor scratch — brand accent */}
        <line x1="-22" y1="44" x2="22" y2="44"
          stroke="var(--accent)" strokeWidth="1.3" />
      </g>

      {/* BOTTOM EMBOSSED LOT NUMBER — steel-embossed effect */}
...
function HeroIconNoose() {
  return (
    <svg viewBox="0 0 120 140" aria-label="noose">
      ...
      {/* hangman's knot — wrapped coil bundle */}
      <g transform="translate(60, 22)">
        {/* bundle silhouette */}
        <path d="M -11 0 Q -14 14 -11 28 L 11 28 Q 14 14 11 0 Z"
              fill="oklch(0.18 0.04 25)" stroke="var(--accent)" strokeWidth="2" />
        {/* the inner rope passing through the bundle (visible above & below) */}
        <line x1="-2.5" y1="0" x2="-2.5" y2="28" stroke="oklch(0.30 0.10 25)" strokeWidth="1.2" />
        {/* coils — diagonal wraps */}
...
function HeroMark({ icon = "none" }) {
  // For the AmEx-style razor blade the blade itself lives next to SYSTEM in the
  // hero title row (see Hero()). Here we just render the big red line.
  if (icon === "amex") {
    return (
  ...
  return (
    <div className="hero-mark">
      {C ? <div className="hero-icon"><C /></div> : null}
      <div className="hero-line" />
...
Object.assign(window, {
  Rails, TallyCounter, BarbedDivider, RazorMark, XMark,
  RazorBlade, RazorSlash, RazorScratch, AmexRazorBlade,
  HeroMark, HERO_ICON_MAP, METHOD_LABELS,
  HeroIconRazor, HeroIconShotgun, HeroIconCapsule, HeroIconNoose, HeroIconTablets,
  IconMatch, IconKnife, IconBottle, IconCassette, IconHourglass,
  RailSystemVert, RailRopeAnim,
});
