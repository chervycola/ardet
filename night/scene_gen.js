...
function genOutside(W,H,opts){
  ...
  function ruinTop(x){
    ...
    return horizon - Math.max(0,rh)*horizon;
  }
  ...
  function groundTone(x,y){
    var fog=Math.max(0,1-(y-horizon)/(H*0.062))*0.58*(0.35+0.65*n2(x*0.02,y*0.31));
  ...
  (function(){
    var idx=0;
    for(var y0=4;y0<horizon-1;y0+=2.6,idx++){
      var th=TH[idx%5], pts=[];
  ...
  (function(){
    var seg=[];
    for(var x=0;x<W;x+=3){
      if(n2(x*0.02,50)>0.42&&skyTone(x,horizon-3)>0.25)seg.push([x,horizon-1+wob(x,horizon,8,0.6)]);
      else{if(seg.length>2)stroke(seg,0.8,'etch');seg=[];}
    }
    if(seg.length>2)stroke(seg,0.8,'etch');
  })();
  ...
  (function(){
    ...
    for(var y=horizon+1.2;y<H;y+=2.3+(y-horizon)*0.036,idx++){
      var th=TH[idx%5],pts=[];
      for(var x=0;x<W;x+=3.0)pts.push([x,y+wob(x,y,2,1.6)]);
      runs(pts,function(px,py){return groundTone(px,py)>th;},groundColor,0.7);
    }
    // tufts of dead grass
    for(var i=0;i<70;i++){
  ...
  (function(){
    ...
    defs.forEach(function(g,i){
      ...
      stroke([[gx,gy],[tx,ty]],3.0,'void');
      stroke([[gx-1.6,gy],[tx-1.6,ty]],0.8,'etch');
      var bx=gx+lean*hgt*0.68, by=gy-hgt*0.68;
      stroke([[bx-hgt*0.32,by-lean*2.5],[bx+hgt*0.32,by+lean*2.5]],2.6,'void');
...
function genTown(W,H){
  ...
  function skyTone(x,y){
    ...
    return Math.max(g*1.1,fog)+(n2(x*0.015,y*0.02)-0.5)*0.14;
  }
  ...
  /* sky: v5 deflected lines around the bone moon + diagonal layer */
  (function(){
    var idx=0;
    for(var y0=4;y0<horizon-1;y0+=2.6,idx++){
      var th=TH[idx%5], pts=[];
      ...
      runs(pts,function(px,py){return py<horizon-1&&inSky(px,py)&&skyTone(px,py)>th;},
        function(px,py){return skyTone(px,py)<0.28?'ash':'etch';},0.7);
    }
    idx=0;
    for(var d0=-H;d0<W;d0+=4.2,idx++){
      var th2=TH[1+(idx%2)],pts2=[];
  ...
  (function(){
    ...
    for(var y=horizon+1.2;y<H;y+=2.3+(y-horizon)*0.036,idxg++){
      ...
      runs(pts,function(px2,py2){
          var fog=Math.max(0,1-(py2-horizon)/(H*0.062))*0.58*(0.35+0.65*n2(px2*0.02,py2*0.31));
          var lamp=Math.max(0,1-Math.hypot(px2-0.225*W,py2-(horizon+(H-horizon)*0.52))/52)*0.75;
          var winl=Math.max(0,1-Math.hypot(px2-win3.x,(py2-horizon-10)*2.2)/60)*0.4;
    ...
    for(var ti=0;ti<46;ti++){
      var tx=hash(ti,41)*W, ty=horizon+6+hash(ti,43)*(H-horizon)*0.8;
