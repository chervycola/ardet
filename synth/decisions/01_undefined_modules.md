...
> ⚠ **SUPERSEDED PARTIALLY (2026-05)** — раздел §3 «And My» в этом документе заменён decision `05_and_my_respec.md`. Принятый ранее вариант A («Last Day ↔ Last Night crossfader») больше не отражает архитектуру модуля. Канонический концепт And My — `../SYSTEM_SUICIDE.md` §8 (MOSFET-shaper из ESC FPV-дрона + выносной пилотный пульт). Остальные разделы документа (Be Careful, Is My, BBAS) остаются в силе.
...
> ⚠ **SUPERSEDED 2026-05** — варианты A/B/C ниже не отражают актуальную архитектуру And My. Принят отдельный концепт: MOSFET-shaper из ESC FPV-дрона + выносной пилотный пульт по MIDI/TRS, edition of 13. См. `05_and_my_respec.md` и `../SYSTEM_SUICIDE.md` §8. Содержимое §3 ниже сохраняется как историческая запись.
