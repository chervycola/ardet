...
>
> ⚠ **РЕКОНТЕКСТУАЛИЗАЦИЯ 2026-05 (Decision 06)** — раздел D «Перекрытие ролей / redundancy analysis» ниже фиксировал, что дублирование сатурации/модуляции/фильтрации между Last Day и ABD/FAS/Be Careful "intentional". С 2026-05 это **зафиксированное намерение архитектуры**, а не побочный эффект: Last Day и Last Night — два standalone-комбайна (anchor-модули, каждый — полная коробка эффектов в одной педали), остальные 7 модулей — сателлиты, чьи роли будут переспеcаться так, чтобы **дополнять, а не дублировать** Day/Night. Конкретный respec сателлитов — отдельный декшн (TBD). См. `../decisions/06_last_day_combine_concept.md` и `../SYSTEM_SUICIDE.md` §7.
