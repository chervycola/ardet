...
|---|--------|---------|-------------------|-----|
| 1 | **I Show You Light** | Осциллятор | Оптическое тон-колесо → re-spec: лазер+FOC (D14) | 16 |
...
> **Slot 1 re-spec 2026-07 (proposed)**: архитектура пересмотрена —
> лазер вместо LED, FOC-сервопетля с магнитным энкодером вместо PLL,
> auto-gearshift по дорожкам, четыре голоса (LIGHT/MIRROR/CLOCK/DUST),
> строб-индикация лока. Полный след: `decisions/14_isyl_respec.md`,
> проектный док: `i_show_you_light/ISYL_PROJECT.md`. Lock после Stage 0.
> Описание ниже — канон v6.5 (LED-версия), сохраняется до lock.
