...
const COLS=[
  ['ЭКСПО /','ДЕМО-ЗОНА'],['ЛЕКЦИИ 1202','РАСТР'],['ЛЕКЦИИ 1203','ВЕКТОР'],
...
const EV=[
  ...
  [0, 50, 70,'g','Вводное слово','и экскурс по программе'],
  [0, 70, 100,'g','Мини-презентация проектов',''],
  [0, 100,110,'g','Экскурсия по МИЭТу',''],
  [0, 110,160,'h','Gogidze',''],
  [0, 160,190,'g','Мини-презентация проектов',''],
  [0, 200,255,'g','Никита Забелин','Как работает Ground'],
  [0, 270,330,'g','Георгий Орлов-Давыдовский','Кастомные инструменты для шумовых перформансов'],
  [0, 360,390,'r','ОРГКОМАНДА','19:00–19:30 · итоги форума · результаты конкурсов'],
  ...
  [1, 240,295,'g','Александр Григорьев\nЗахар Лунев','DSP-платформа «Семечко» и Rostok IDE'],
  [1, 300,355,'g','Илья Чертков','Творческое применение самодельных микрофонов'],
  [2, 60, 115,'g','Евгений Карло','Цена и ценность музыкальных устройств'],
  [2, 120,175,'g','Иван Антоновский','Микроконтроллеры и их применение — в технике, в жизни, в музыке'],
  [2, 180,235,'g','Игорь Мирзоев','3D-печать в музыкальной электронике · Пикасо 3D'],
  [2, 240,295,'g','Владимир Черепанов','Медицинский звук: непрофильная техника'],
...
function cell(c,t1,t2,st,title,desc,tl){
  ...
  const short = (t2-t1)<=15;
  const ry = short ? y-2 : y, rh = short ? h+4 : h;
  let out=`<rect x="${x}" y="${ry.toFixed(1)}" width="${w.toFixed(1)}" height="${rh.toFixed(1)}" fill="${fill}" stroke="#000" stroke-width="1.6"/>\n`;
  const pad=x+10, maxY=y+h-6;
  if(short){
    out+=`<text x="${pad}" y="${y+h/2+6}" font-weight="800" font-size="17" fill="${tc}"><tspan font-family="'Courier New',monospace" font-weight="400" font-size="13.5" fill="${timc}">${fmt(t1)}</tspan> ${esc(title)}</text>\n`;
    return out;
  }
  if(t2-t1<=25){
    out+=`<text x="${pad}" y="${y+18}" font-family="'Courier New',monospace" font-size="13.5" letter-spacing="0.5" fill="${timc}">${tl || (fmt(t1)+' – '+fmt(t2))}</text>\n`;
    out+=`<text x="${pad}" y="${y+37}" font-weight="800" font-size="17" fill="${tc}">${esc(title)}</text>\n`;
    if(desc) out+=`<text x="${pad}" y="${y+52}" font-size="12.5" fill="${tc}">${esc(desc)}</text>\n`;
    return out;
  }
  let ty=y+24;
  if(st!=='r'){
    out+=`<text x="${pad}" y="${ty}" font-family="'Courier New',monospace" font-size="16" letter-spacing="0.5" fill="${timc}">${tl || (fmt(t1)+' – '+fmt(t2))}</text>\n`;
    ty+=25;
  }
